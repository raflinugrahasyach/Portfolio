...
def main():
    ...
    # Fix corrupted floating point noise in categorical/ordinal columns
    df_day_clean['yr'] = df_day_clean['dteday'].dt.year
    df_hour_clean['yr'] = df_hour_clean['dteday'].dt.year
    df_hour_clean['hr'] = df_hour_clean['hr'].round().clip(0, 23).astype(int)
    if pd.api.types.is_numeric_dtype(df_day_clean['weathersit']):
        df_day_clean['weathersit'] = df_day_clean['weathersit'].round().clip(1, 4).astype(int).astype(str)
